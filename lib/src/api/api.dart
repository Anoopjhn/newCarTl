...
class ApiUtils {
  ...
  static final baseUrl = "http://webartistictech.com/popular_evaluator/mobile/evaluator/v1/get_details.php";
  ...
  Future<RespObj> getData(String route,{String header}) async{
    ...
    try{
      Response response = await get(Uri.parse(baseUrl + route),headers: mHeaders);
  ...
  Future<RespObj> postData(String route,{String mBody,String header}) async{
    ...
    try{
      Response response = await post(Uri.parse(baseUrl + route),headers: mHeaders,body: mBody);
      if(response.statusCode==200){
        return RespObj.fromJSON(json.decode(response.body));
      }else{
        return RespObj.fromJSON(json.decode(response.body));
